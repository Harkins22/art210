...
void setup()
{
  ...
  bg = loadImage("bg.jpg");
  f = createFont("GhostyFont.ttf",100);
...
void startScreen()
{
  ...
  textFont(f);
  textSize(100);
  text("Ghosty Run",width*.5,height*.33);
  textSize(30);
  text("Press ANY key to Run",width/2,height/2);
...
void deadScreen()
{
  ...
  fill(255);
  textAlign(CENTER,CENTER);
  text(" :(  Press ANY key to run again",width/2,height/2);
  _jumpCount = 0;
}
